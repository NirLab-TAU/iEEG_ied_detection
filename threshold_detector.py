...
for subj in range(1, 26):
    id = subj if subj >= 10 else f'0{subj}'
    # Define the path to the subject's data
    dir = Path(fr'.\ieeg_ieds_bids\sub-{id}\ieeg\\')
    fname = PurePath(dir, f"sub-{id}_task-sleep_ieeg.edf")
    ...
    spikes_df.to_csv(PurePath(fr'.\thresh\\', f'P{id}_spikes.csv'))
